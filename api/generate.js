...
import { PDFDocument } from 'pdf-lib'; // or use reportlab/python side
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: apiKeyRecord } = await supabase
      ...
      .eq('api_key', apiKey)
      .single();
    if (!apiKeyRecord) return res.status(403).json({ error: 'Forbidden. Invalid API Key.' });
