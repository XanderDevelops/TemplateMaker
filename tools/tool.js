...
async function loadCachedData() {
    ...
    if (fileName && fileData) {
        ...
        processFileData(arrayBuffer, fileName, { skipIdentifierModal: true, skipActivityLog: true });
    }
}
...
const adders = {
    text: (x, y, content = (window.CSVLinkI18n?.t('Sample Text') || 'Sample Text')) => {
...
function renderMultiSelectInspector(selection) {
    ...
    if (allText) {
        ...
        multiInspector.appendChild(section('Text Formatting', [
            ...
            buttonGroupRow('Text Casing', [
                { value: 'uppercase', label: 'AA' },
                { value: 'lowercase', label: 'aa' },
                { value: 'capitalize', label: 'Aa' },
                { value: 'none', label: '[/]' }
            ], null, (v) => {
                objects.forEach(obj => window.applyTextCasing && window.applyTextCasing(obj, v));
                canvas.renderAll();
                requestSaveState();
                refreshInspector({ target: selection });
            }),
    ...
    const commonFields = [];
    if (!allText) {
        commonFields.push(colorInputRow('Fill Color', getCommonPropertyValue(objects, 'fill') ?? '', v => setCommonPropertyValue(objects, 'fill', v)));
    }
    commonFields.push(
    ...
    multiInspector.appendChild(section('Common', commonFields));
}



window.applyTextCasing = function (obj, casing) {
    if (!obj || obj.type !== 'textbox') return;
    let text = obj.text || '';
    if (casing === 'uppercase') text = text.toUpperCase();
    else if (casing === 'lowercase') text = text.toLowerCase();
    else if (casing === 'capitalize') {
        text = text.split(/(\s+)/).map(w => w.length > 0 && w.trim() ? w[0].toUpperCase() + w.slice(1).toLowerCase() : w).join('');
    }
    if (casing !== 'none') {
        obj.set({ text });
        obj.initDimensions();
    ...
    if (o.type === 'textbox') {
        inspector.appendChild(section('Text', [
            ...
            buttonGroupRow('Text Style', [
                ...
                if (typeof refreshTextboxCurve === 'function') refreshTextboxCurve(o, { skipRender: true });
                canvas.renderAll();
                requestSaveState();
                refreshInspector({ target: o });
            }),
            buttonGroupRow('Text Casing', [
                ...
                if (window.applyTextCasing) window.applyTextCasing(o, v);
                canvas.renderAll();
                requestSaveState();
                refreshInspector({ target: o });
            }),
