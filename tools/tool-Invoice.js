...
function positionInvoiceFieldsHelper() {
    ...
    const highlight = $('#invoiceFieldsHelperHighlight');
    const modal = $('#invoiceFieldsHelperModal');
    if (!panel || !highlight || !modal || panel.style.display === 'none') {
    ...
    let left = rect.left - modalRect.width - 18;
    if (left < 16) left = rect.right + 18;
    if (left + modalRect.width > window.innerWidth - 16) {
        left = Math.max(16, window.innerWidth - modalRect.width - 16);
    }
...
function queueInvoiceFieldsHelper() {
    pendingInvoiceFieldsHelper = true;
    window.requestAnimationFrame(() => {
        window.requestAnimationFrame(() => {
...
function flushInvoiceFieldsHelper() {
    if (!pendingInvoiceFieldsHelper) return;
    window.setTimeout(() => {
        if (pendingInvoiceFieldsHelper) showInvoiceFieldsHelper();
    }, 140);
}
...
function renderInvoiceCsvFieldsPanel() {
    ...
    headers.forEach((headerName) => {
        ...
        chip.type = 'button';
        chip.className = 'invoice-csv-field-chip';
        chip.draggable = true;
        chip.innerHTML = `<strong>${escapeHtml(String(headerName))}</strong><span>${dataRows.length.toLocaleString()} row(s)</span>`;
        chip.addEventListener('dragstart', (event) => {
            if (!event.dataTransfer) return;
            event.dataTransfer.effectAllowed = 'copy';
            event.dataTransfer.setData('text/plain', `csv-field:${headerName}`);
        });
        chip.addEventListener('click', () => {
            const spawn = getDefaultSpawnPoint();
            addLinkedCsvFieldToCanvas(headerName, spawn.x, spawn.y);
        });
...
async function restoreEditorHandoff(handoff) {
    ...
    hideInvoicePdfToggleHelper();
    hideInvoiceAiHelper();
    syncInvoiceCsvPromptState({ forceOpen: false });
...
async function initializeEditor() {
    ...
    syncInvoiceCsvPromptState({ forceOpen: !editorHandoff && !hasInvoiceTableStructure() });
...
async function loadTemplateFromDB(templateId, options = {}) {
    ...
    if (isPublic) {
        const { data: publicData, error: publicError } = await supabase.from('public_templates').select('id, title, template_data, preview_url').eq('id', templateId).single();
...
function initializeCanvas() {
    ...
    const savedCanvasPagesPanelState = localStorage.getItem(PAGE_STRIP_COLLAPSE_KEY);
    isCanvasPagesPanelCollapsed = savedCanvasPagesPanelState == null ? true : savedCanvasPagesPanelState === '1';
...
function unloadData() {
    ...
    updateFloatingLinker(null);
    renderCsvView();
    syncInvoiceCsvPromptState({ forceOpen: true });
...
async function loadCachedData() {
    ...
    if (fileName && fileData) {
        ...
        processFileData(arrayBuffer, fileName, { skipIdentifierModal: true, showFieldsHelper: false });
    }
}
...
const adders = {
    text: (x, y, content = (window.CSVLinkI18n?.t('Sample Text') || 'Sample Text')) => canvas.add(new fabric.Textbox(content, { left: x, top: y, width: 180, fontSize: 28, fill: '#000000', fontFamily: 'Arial', originX: 'center', originY: 'center', styles: [], padding: 0, curveAmount: 0, name: getUniqueName('text'), lockUniScaling: true, pageId: currentCanvasPageId() })).setActiveObject(canvas.getObjects().pop()),
...
function addLinkedCsvFieldToCanvas(columnName, x, y) {
    const safeColumnName = String(columnName || '').trim();
    if (!safeColumnName) return null;

    const placeholderText = `{{${safeColumnName}}}`;
    const textWidth = Math.min(Math.max(placeholderText.length * 11, 170), 420);
    const textColor = getComputedStyle(document.body).getPropertyValue('--fg').trim() || '#0f172a';
    const textObject = new fabric.Textbox(placeholderText, {
        left: x,
        top: y,
        width: textWidth,
        fontSize: 24,
    ...
    saveBinding(textObject, { column: safeColumnName, property: 'Text Content' });
    canvas.requestRenderAll();
    updateFloatingLinker(textObject);
...
dropTarget.addEventListener('drop', (e) => {
    ...
    else if (dataType === 'csv-field') { addLinkedCsvFieldToCanvas(value, x, y); }
});
...
// components/04-settings-inspector-binding.js
        ...
        function guessPreferredIdentifierColumn() {
            if (identifierColumn && headers.includes(identifierColumn)) return identifierColumn;
            const patterns = [
        ...
        function getIdentifierColumnStats(columnName) {
            if (!columnName) {
                return {
                    uniqueCount: dataRows.length,
                    multiRowCount: 0
                };
            }
            const groupedValues = [...new Set(dataRows.map(row => row[columnName]).filter(Boolean))];
            const multiRowCount = groupedValues.filter(value => dataRows.filter(row => row[columnName] === value).length > 1).length;
            return {
                uniqueCount: groupedValues.length,
                multiRowCount
            };
        }
        ...
        function setIdentifierModeSelection(mode = '') {
            ...
            const groupingPanel = $('#identifierGroupingPanel');
            const select = $('#identifierColumnSelect');

            singleCard?.classList.toggle('is-selected', mode === 'single');
            groupedCard?.classList.toggle('is-selected', mode === 'grouped');
            groupingPanel?.classList.toggle('is-disabled', mode !== 'grouped');

            if (select) {
                select.disabled = mode !== 'grouped';
            }
        ...
        function updateIdentifierConfirmButton() {
            const button = $('#confirmIdentifierColumnBtn');
            const modal = $('#identifierColumnModal');
            const select = $('#identifierColumnSelect');
            if (!button) return;
            ...
            if (selectedMode === 'single') {
                button.disabled = false;
                button.textContent = 'Use One Row Per Invoice';
                return;
            }
